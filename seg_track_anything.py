...
def tracking_objects_in_video(SegTracker, input_video, frame_num=0):
    ...
    io_args = {
        ...
        'output_h264': f'{tracking_result_dir}/{video_name}_seg1.mp4',
    }
...
def video_type_input_tracking(SegTracker, input_video, io_args, video_name, frame_num=0):
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened():
            ...
            cv2.imwrite(f"{io_args['nomal_frame_dir']}/{str(frame_idx).zfill(5)}.jpg", frame[:, :, ::-1])
    ...
    torchvision.io.write_video(io_args['output_h264'], frames, fps=fps, video_codec="h264")
    
    return io_args['output_h264'], f"{io_args['tracking_result_dir']}/{video_name}_pred_mask.zip"